...
{
    /// <summary>
    /// Клас реалізує метод хорд (метод січних) з фіксованим кінцем відрізку,
    /// що вибирається згідно з правилом правильної випуклості 
    /// (ознака збіжності: φ(x) * φ''(x) &gt; 0).
    ///
    /// Метод розв’язує рівняння f(x) = g(x) на заданому інтервалі [a, b]
    /// шляхом послідовного наближення змінної точки до кореня.
    /// </summary>
